import { createFeatureFlag } from "@/lib/config/flags";
...
function FeatureFlagCard({ name, enabled, description }: FeatureFlagCardProps) {
  return (
    <div className={`bg-white rounded-lg shadow-lg p-6 border-l-4 ${enabled ? 'border-l-green-500' : 'border-l-red-500'
      }`}>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-xl font-semibold text-gray-900">
          {name}
        </h3>
        <span className={`px-4 py-1 rounded-full text-sm font-semibold ${enabled
