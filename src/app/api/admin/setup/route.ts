...
export async function GET(request: NextRequest) {
  try {
    ...
    // Checking if SystemSettings has we might be fetching the first record of the table which is bad.
    ...
    // For now, let's assume `` handles it if we added it to the intercepted models list?
    // In ``, SystemSettings is NOT in the list of tenantModels. 
    // So `` won't automatically auto-inject tenantId for SystemSettings yet.
    ...
    // and `` doesn't include it, we will assume for now it's single tenant behavior OR 
    // we use `db` but since it's not intercepted, it acts like normal prisma.
    // Wait, if it's SaaS, "School Name" etc. MUST be per tenant.
    // If SystemSettings doesn't have we are in trouble.
    // Let's assume we will add it or it exists.
    
    // For now, let's stick to using `` so that if we update the `tenantModels` list later, it just works.
