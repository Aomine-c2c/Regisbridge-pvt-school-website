        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-center mb-8 space-x-4">
            <button
              ...
              className={`px-6 py-3 rounded-xl font-semibold transition-colors ${activeTab === 'primary'
                  ? 'bg-[#1C1A75] text-white'
                  : 'bg-[#FFFDF7] text-[#1C1A75] border border-[#1C1A75]/10 hover:bg-[#FFF7E6]'
                }`}
            >
              Primary (Grades 1-7)
            </button>
            <button
              ...
              className={`px-6 py-3 rounded-xl font-semibold transition-colors ${activeTab === 'secondary'
                  ? 'bg-[#1C1A75] text-white'
                  : 'bg-[#FFFDF7] text-[#1C1A75] border border-[#1C1A75]/10 hover:bg-[#FFF7E6]'
                }`}
            >
          ...
          <div className="glass-card p-8 rounded-lg shadow-strong mb-8 border border-white/20">
            {activeTab === 'primary' ? (
              <div>
                ...
                <div className="gradient-gold bg-opacity-10 border border-[#D4AF37] p-4 rounded-lg mb-4 shadow-soft card-elevated">
                  <h4 className="font-bold text-[#1C1A75] mb-2">Foreign Language Programs</h4>
                  ...
                  <div className="grid md:grid-cols-2 gap-2 text-sm">
                    {classBranding.map((classInfo, idx) => (
                      <div key={idx} className="flex items-start space-x-2">
                        ...
                        <div>
                          <span className="font-semibold text-[#1C1A75]">{classInfo.grade}:</span>{' '}
                          <span className="text-gray-700">{classInfo.birdName}</span>
                          <div className="text-xs text-gray-600 italic">"{classInfo.symbolism}"</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="gradient-dawn bg-opacity-10 border border-purple-200 p-4 rounded-lg mb-4 shadow-soft card-elevated">
                  <h4 className="font-bold text-[#1C1A75] mb-3 text-center">School Blocks - Named After Great Inventors</h4>
                  <div className="grid md:grid-cols-2 gap-3 text-sm">
                    {blockBranding.map((block, idx) => (
                      <div key={idx} className="glass-card p-3 rounded shadow-sm micro-float" style={{ animationDelay: `${idx * 0.05}s` }}>
                        <div className="font-bold text-[#1C1A75]">{block.newName}</div>
                        <div className="text-xs text-gray-600">{block.blockName}</div>
                  ...
                  <div className="flex justify-center space-x-6">
                    {sportsHouses.map((house, idx) => (
                      <div key={idx} className="text-center micro-wiggle">
                        ...
                        <div className="font-semibold text-[#1C1A75]">{house}</div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {subjects.map((subject, index) => (
              <div
                key={subject.name}
                className="glass-card p-6 rounded-lg shadow-medium text-center card-elevated transition-all duration-300 cursor-pointer group animate-scale-in border border-white/20"
                onClick={() => handleSubjectClick(subject)}
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <subject.icon className="mx-auto mb-3 text-[#1C1A75] group-hover:text-[#D4AF37] micro-wiggle transition-all duration-300" size={40} />
              ...
              <div className="glass-card rounded-lg max-w-md w-full animate-scale-in shadow-strong border border-white/30">
                <div className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      onClick={closeModal}
                      className="text-gray-400 hover:text-gray-600 transition-colors micro-bounce"
