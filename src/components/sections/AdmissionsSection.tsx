...
export default function AdmissionsSection() {
  ...
  return (
    <section id="admissions" className="py-20 md:py-24 bg-gradient-to-b from-white to-gray-50">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="glass-card p-8 rounded-lg shadow-strong mb-8 border border-white/20">
            ...
            <div className="gradient-gold bg-opacity-10 border border-[#D4AF37] p-4 rounded-lg mb-6 shadow-soft card-elevated">
              <h4 className="font-bold text-[#1C1A75] mb-3">Transport Service</h4>
              <div className="mb-3">
                <span className="text-2xl font-bold text-gradient-gold">${transportFees.monthly}/month</span>
                <span className="text-sm text-gray-600 ml-2">(Payment due by 3rd of each month)</span>
              </div>
                ...
                {transportSchedule.map((schedule, idx) => (
                  <div key={idx} className="glass-card p-3 rounded shadow-medium micro-float" style={{ animationDelay: `${idx * 0.05}s` }}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-bold text-[#1C1A75]">{schedule.level}</span>
                      <span className="text-xs gradient-primary text-white px-2 py-1 rounded">{schedule.pickupArea}</span>
                    </div>
                    <div className="text-sm text-gray-700">🕐 {schedule.pickupTime}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 text-gray-700">
              <div className="glass-card p-4 rounded-lg shadow-soft">
                <h4 className="font-semibold mb-2 text-[#1C1A75]">Application Requirements:</h4>
              ...
              <div className="glass-card p-4 rounded-lg shadow-soft">
                <h4 className="font-semibold mb-2 text-[#1C1A75]">What We Offer:</h4>
                <ul className="list-disc list-inside space-y-1">
